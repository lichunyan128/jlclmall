		...
		function n(e) {
			...
			function u(e, t) {
				...
				for(l = 0; h > l; ++l) {
					...
					e[f] = I + F, e[f + 4] = I - F;
					var D = .707106781 * (U + P);
					e[f + 2] = P + D, e[f + 6] = P - D, I = b + y, F = y + w, U = w + p;
					var C = .382683433 * (I - U),
						T = .5411961 * I + C,
						x = 1.306562965 * U + C,
						L = .707106781 * F,
				...
				for(f = 0, l = 0; h > l; ++l) {
					...
					e[f] = W + H, e[f + 32] = W - H;
					var q = .707106781 * (z + V);
					e[f + 16] = V + q, e[f + 48] = V - q, W = N + j, H = j + k, z = k + _;
					var X = .382683433 * (W - z),
						J = .5411961 * W + X,
						Y = 1.306562965 * z + X,
						Q = .707106781 * H,
			...
			function p(e, t, n, r, i) {
				...
				var p = A[0] - n;
				n = A[0], 0 == p ? a(r[0]) : (o = 32767 + p, a(r[L[o]]), a(x[o]));
				for(var m = 63; m > 0 && 0 == A[m]; m--);
				if(0 == m) return a(s), n;
				for(var w, v = 1; m >= v;) {
					for(var y = v; 0 == A[v] && m >= v; ++v);
					var S = v - y;
					if(S >= l) {
						w = S >> 4;
			...
			this.encode = function(e, t, n) {
				...
				for(var u, m, v, P, F, U, D, x, L, R = e.data, A = e.width, j = e.height, N = 4 * A, W = 0; j > W;) {
					for(u = 0; N > u;) {
						for(F = N * W + u, U = F, D = -1, x = 0, L = 0; 64 > L; L++) x = L >> 3, D = 4 * (7 & L), U = F + x * N + D, W + x >= j && (U -= N * (W + 1 + x - j)), u + D >= N && (U -= u + D - N + 4), m = R[U++], v = R[U++], P = R[U++], E[L] = (B[m] + B[v + 256 >> 0] + B[P + 512 >> 0] >> 16) - 128, k[L] = (B[m + 768 >> 0] + B[v + 1024 >> 0] + B[P + 1280 >> 0] >> 16) - 128, O[L] = (B[m + 1280 >> 0] + B[v + 1536 >> 0] + B[P + 1792 >> 0] >> 16) - 128;
						i = p(E, C, i, y, b), o = p(k, T, o, S, I), s = p(O, T, s, S, I), u += 32
					}
					W += 8
				}
				if(_ >= 0) {
